...
async function loadRules() {
  try {
    ...
    rules = Array.isArray(data) ? data : (data.data || data.rules || []);
    renderRules();
  } catch(e) {
...
function renderRules() {
  ...
  document.getElementById('s-allow').textContent  = rules.filter(r=>r.action==='allow').length;
  document.getElementById('s-deny').textContent   = rules.filter(r=>r.action==='deny').length;
  ...
  tbody.innerHTML = rules.map(r => {
    const id   = r.id;
    const zips = (r.zipCodes||[]).join(', ') || '—';
    const act  = r.action || 'allow';
    const ena  = r.status !== 'paused';
    return \`<tr>
      <td><input type="checkbox" class="row-chk" data-id="\${id}" onchange="toggleSel('\${id}',this.checked)"/></td>
      <td class="mono">\${zips}</td>
      <td><span class="badge badge-\${act}"><span class="dot"></span>\${act==='allow'?'Allow':'Deny'}</span></td>
      ...
      <td style="display:flex;gap:6px">
        <button class="btn btn-ghost btn-sm" onclick="toggleRule('\${id}', \${!ena})">\${ena?'⏸️ Disable':'▶️ Enable'}</button>
...
async function addRule() {
  ...
  if (!/^\d{5}(-\d{4})?$/.test(zip)) { toast('Enter a valid 5-digit zip code', 'e'); return; }
  try {
    const res = await fetch(API+'/api/rules', {
