...
// ── App State ─────────────────────────────────────────────────────────────────
let appActive   = true;
let currentPlan = "free"; // free | basic | starter | pro
let customCSS   = "";
...
app.get("/widget.js", (req, res) => {
  ...
  if(!${appActive})return;
  var API='${HOST}';
  var _css='${customCSS.replace(/'/g, "\\'").replace(/\n/g, " ")}';
  function addStyles(){
    if(document.getElementById('zc-styles'))return;
    var st=document.createElement('style');st.id='zc-styles';
    st.textContent='[data-zipcheck] .zc-wrap{font-family:-apple-system,BlinkMacSystemFont,sans-serif;margin:12px 0;padding:16px;border:1px solid #e4e5e7;border-radius:10px;background:#fff}[data-zipcheck] .zc-lbl{display:block;font-weight:700;font-size:14px;margin-bottom:10px;color:#1a1a1a}[data-zipcheck] .zc-row{display:flex;gap:8px;flex-wrap:wrap}[data-zipcheck] .zc-inp{flex:1;min-width:120px;padding:9px 12px;border:1.5px solid #c9cccf;border-radius:8px;font-size:15px;outline:none;font-family:inherit;transition:border-color .15s}[data-zipcheck] .zc-inp:focus{border-color:${btnColor}}[data-zipcheck] .zc-btn{padding:9px 20px;background:${btnColor};color:${btnTxt};border:none;border-radius:8px;font-weight:700;font-size:14px;cursor:pointer;transition:opacity .15s}[data-zipcheck] .zc-btn:hover{opacity:.85}[data-zipcheck] .zc-res{margin-top:10px;font-size:14px;min-height:18px;font-weight:500;line-height:1.5}';
    document.head.appendChild(st);
    if(_css){var cu=document.createElement('style');cu.id='zc-custom';cu.textContent=_css;document.head.appendChild(cu);}
  }
  function hideCartBtns(){['[name=add]','[data-testid=add-to-cart]','.product-form__submit','[data-action=add-to-cart]','#AddToCart','#add-to-cart-btn','.shopify-payment-button','[data-testid=BuyNow]'].forEach(function(sel){document.querySelectorAll(sel).forEach(function(el){el.style.display='none';el.setAttribute('data-zc-hidden','1');});});}
  function showCartBtns(){document.querySelectorAll('[data-zc-hidden]').forEach(function(el){el.style.display='';el.removeAttribute('data-zc-hidden');});}
  function build(el,i){
    addStyles();
    var label=el.getAttribute('data-label')||'${title}';
    var ph=el.getAttribute('data-placeholder')||'${ph}';
    var btn=el.getAttribute('data-btn-text')||'${btnLbl}';
    var ii='zci'+i,bi='zcb'+i,ri='zcr'+i;
    el.innerHTML='<div class="zc-wrap"><span class="zc-lbl">\\u{1F4CD} '+label+'</span><div class="zc-row"><input class="zc-inp" id="'+ii+'" placeholder="'+ph+'" maxlength="12"/><button class="zc-btn" id="'+bi+'">'+btn+'</button></div><div class="zc-res" id="'+ri+'"></div></div>';
    hideCartBtns();
    async function chk(){
      ...
      try{
        ...
        var d=j.data;
        if(d.result==='allow'){out.innerHTML='<span style="color:${okColor}">\\u2705 '+(d.message||'${okMsg}')+'</span>';showCartBtns();}
        else if(d.result==='block'||d.result==='deny'){out.innerHTML='<span style="color:${errColor}">\\u{1F6AB} '+(d.message||'${errMsg}')+'</span>';hideCartBtns();}
        else{out.innerHTML='<span style="color:#f59e0b">\\u2139\\uFE0F No rule found. Please contact us.</span>';}
  ...
  function autoPlace(){
    var selectors=['.product-form__submit','[name=add]','#AddToCart','.shopify-payment-button__button','[data-testid=add-to-cart]'];
    var target=null;
    for(var i=0;i<selectors.length;i++){target=document.querySelector(selectors[i]);if(target)break;}
    if(!target||document.querySelector('[data-zipcheck]'))return;
...
body{font-family:var(--font);background:var(--g50);color:var(--g900);height:100vh;display:flex;overflow:hidden;-webkit-font-smoothing:antialiased}

/* SIDEBAR */
.sidebar{width:224px;background:var(--white);border-right:1px solid var(--g200);display:flex;flex-direction:column;flex-shrink:0;overflow-y:auto;height:100vh}
.sidebar-brand{padding:16px 14px 12px;border-bottom:1px solid var(--g100);display:flex;align-items:center;gap:10px;flex-shrink:0}
...
.sidebar-footer{padding:10px 8px 14px;border-top:1px solid var(--g100);flex-shrink:0}
.app-status-pill{display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:8px;background:var(--g50);border:1px solid var(--g200)}
.status-dot{width:8px;height:8px;border-radius:50%;background:var(--green);flex-shrink:0;box-shadow:0 0 0 2px rgba(0,166,126,.2)}
...
.status-slider{position:absolute;inset:0;background:var(--g300);border-radius:20px;transition:.2s}
...
.page{display:none}.page.active{display:block;max-width:900px}
...
.cnt{background:var(--g100);color:var(--g600);font-size:11px;padding:2px 8px;border-radius:20px;font-weight:600}
...
/* SETTINGS */
.settings-2col{display:grid;grid-template-columns:1fr 320px;gap:20px;align-items:start}
...
.preview-tabs{display:flex;border-bottom:1px solid var(--g200);background:var(--g50)}
.preview-tab{flex:1;padding:9px;border:none;background:none;font-size:12px;font-weight:600;color:var(--g500);cursor:pointer;font-family:var(--font);transition:all .15s;border-bottom:2px solid transparent}
.preview-tab.active{color:var(--green-dk);border-bottom-color:var(--green);background:var(--white)}
.preview-body{padding:16px}
.preview-desktop{width:100%}
.preview-mobile{width:320px;margin:0 auto;border:8px solid var(--g900);border-radius:20px;overflow:hidden;background:#fff}
.preview-mobile-inner{padding:12px}
.preview-widget{border:1px solid var(--g200);border-radius:10px;padding:16px;background:#fff}

/* CUSTOM CSS */
.css-editor{width:100%;min-height:300px;font-family:var(--mono);font-size:13px;padding:16px;border:1.5px solid var(--g200);border-radius:8px;background:#0f172a;color:#7dd3fc;outline:none;resize:vertical;line-height:1.7;transition:border-color .15s}
.css-editor:focus{border-color:var(--green)}

/* APP BLOCK */
.appblock-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;padding:20px}
.appblock-card{border:2px solid var(--g200);border-radius:12px;padding:20px;cursor:pointer;transition:all .2s;text-align:center}
.appblock-card:hover{border-color:var(--green);background:var(--green-lt)}
.appblock-card.selected{border-color:var(--green);background:var(--green-lt)}
.appblock-icon{font-size:36px;margin-bottom:10px}
.appblock-name{font-size:14px;font-weight:700;margin-bottom:4px}
.appblock-desc{font-size:12px;color:var(--g500);line-height:1.5}
...
.ptl{font-size:15px;font-weight:600;color:var(--g500)}.ptl.active{color:var(--g900)}
...
.plan-card{background:var(--white);border:2px solid var(--g200);border-radius:16px;padding:22px 18px;position:relative;transition:all .2s}
...
.plan-card.current-plan{border-color:var(--blue);background:var(--blue-lt)}
.popular-badge{position:absolute;top:-13px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#f093fb,#ee0979);color:#fff;font-size:11px;font-weight:700;padding:4px 14px;border-radius:20px;white-space:nowrap}
.current-badge{position:absolute;top:-13px;left:50%;transform:translateX(-50%);background:var(--blue);color:#fff;font-size:11px;font-weight:700;padding:4px 14px;border-radius:20px;white-space:nowrap}
.plan-ent-label{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--purple);margin-bottom:4px}
.plan-name{font-size:18px;font-weight:800;color:var(--g900);margin-bottom:6px}
.plan-desc{font-size:12px;color:var(--g500);margin-bottom:14px;line-height:1.5;min-height:34px}
.plan-price{display:flex;align-items:flex-end;gap:2px;margin-bottom:2px}
.plan-curr{font-size:15px;font-weight:700;color:var(--g700);line-height:1.9}
.plan-amt{font-size:34px;font-weight:800;color:var(--g900);line-height:1;letter-spacing:-1px}
.plan-period{font-size:13px;color:var(--g500);line-height:2.2}
.plan-billed{font-size:11px;color:var(--g400);margin-bottom:14px;min-height:16px}
.plan-btn{width:100%;padding:10px;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;border:none;font-family:var(--font);transition:all .15s;margin-bottom:18px}
...
.plan-btn-current{background:var(--g100);color:var(--g500);cursor:default;border:none}
.plan-features{list-style:none;display:flex;flex-direction:column;gap:7px}
.plan-features li{font-size:12px;color:var(--g700);display:flex;align-items:flex-start;gap:7px;line-height:1.4}
.plan-features li .fi{flex-shrink:0;margin-top:1px}
.plan-features li.no{color:var(--g400);text-decoration:line-through}
.plan-divider{border:none;border-top:1px solid var(--g100);margin:12px 0}
.plan-limit-banner{background:var(--amber-lt);border:1px solid #fde68a;border-radius:10px;padding:14px 18px;margin-bottom:16px;display:flex;align-items:center;gap:12px;font-size:13px;color:var(--amber)}
...
@media(max-width:900px){.plans-grid{grid-template-columns:repeat(2,1fr)}.settings-2col{grid-template-columns:1fr}}
@media(max-width:640px){.sidebar{display:none}.stats{grid-template-columns:1fr 1fr}.plans-grid{grid-template-columns:1fr}}
</style></head><body>
...
<div class="page active" id="page-rules">
  <div class="page-header"><div class="page-title">Zip Code Rules</div><div class="page-sub">Add zip / postal codes and mark Allow or Deny. Widget checks these in real time.</div></div>
  <div id="plan-limit-banner" style="display:none" class="plan-limit-banner">⚠️ <span id="plan-limit-msg"></span> <button class="btn btn-sm btn-primary" style="margin-left:auto" onclick="nav(document.querySelector('[onclick*=pricing]'),'pricing')">Upgrade Plan</button></div>
  <div class="stats">
  ...
  <div class="card">
    ...
    <div class="form-row">
      <div class="fld" style="max-width:170px"><label>Zip / Postal Code</label><input id="f-zip" placeholder="e.g. 10001" maxlength="12"/></div>
  ...
  <div class="card">
    ...
    <div style="padding:16px 20px;display:flex;flex-wrap:wrap;gap:14px;align-items:center">
      <div><div style="font-size:11px;font-weight:700;color:var(--g500);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px">Export All</div><div style="display:flex;gap:8px"><button class="btn btn-ghost btn-sm" onclick="exportRules('csv')">⬇️ CSV</button><button class="btn btn-ghost btn-sm" onclick="exportRules('xlsx')">⬇️ Excel</button></div></div>
      <div style="width:1px;height:36px;background:var(--g200)"></div>
      <div><div style="font-size:11px;font-weight:700;color:var(--g500);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px">Bulk Import</div><button class="btn btn-primary btn-sm" onclick="checkImportPlan()">⬆️ Upload CSV / Excel</button></div>
...
<div class="page" id="page-settings">
  <div class="page-header"><div class="page-title">Widget Settings</div><div class="page-sub">Changes reflect live in the preview. Save to apply to your storefront.</div></div>
  <div class="card">
        ...
        <div class="preview-panel">
          <div class="preview-card">
            ...
            <div class="preview-body">
              <div id="pv-desktop" class="preview-desktop">
                <div class="preview-widget">
                  ...
                  <div style="display:flex;gap:8px;margin-bottom:10px">
                    ...
                    <button id="pv-btn" style="padding:9px 14px;background:#008060;color:#fff;border:none;border-radius:8px;font-weight:700;font-size:13px;cursor:pointer;white-space:nowrap">Check</button>
                  </div>
              ...
              <div id="pv-mobile" style="display:none">
                <div class="preview-mobile">
                  <div class="preview-mobile-inner">
                    ...
                    <input id="pvm-input" placeholder="Enter zip / postal code" style="width:100%;padding:8px 10px;border:1.5px solid #c9cccf;border-radius:8px;font-size:13px;outline:none;margin-bottom:8px;box-sizing:border-box" readonly/>
                    <button id="pvm-btn" style="width:100%;padding:9px;background:#008060;color:#fff;border:none;border-radius:8px;font-weight:700;font-size:13px;cursor:pointer">Check</button>
              ...
              <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--g100)">
                <div style="font-size:11px;color:var(--g400);font-weight:600;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Live on storefront after Save</div>
                <div style="font-size:12px;color:var(--g500)">All color and text changes sync to your widget automatically.</div>
              </div>
            </div>
          </div>
        </div>
...
<div class="page" id="page-analytics">
  <div class="page-header"><div class="page-title">Analytics</div><div class="page-sub">Every zip code check by your customers, in real time.</div></div>
  <div class="card">
...
<div class="page" id="page-embed">
  <div class="page-header"><div class="page-title">Embed &amp; Shortcode</div><div class="page-sub">Add the zip checker anywhere. You control placement — no auto-injection.</div></div>
...
&lt;script src="${HOST}/widget.js" async&gt;&lt;/script&gt;</div>
      <div class="info-box">💡 The widget also auto-inserts above Add to Cart when the app is active. Manual placement gives you full control.</div>
    </div>
  </div>
  <div class="card"><div class="card-head"><h2>🌐 iFrame — Any Website</h2></div>
    <div style="padding:16px 20px"><div class="code-block" id="c2"><button class="copy-btn" onclick="cc('c2')">Copy</button>&lt;iframe src="${HOST}/embed" width="100%" height="160" frameborder="0" style="border-radius:10px;border:none"&gt;&lt;/iframe&gt;</div></div>
  </div>
...
add_shortcode('zipcheck', 'zipcheck_widget');</div>
  ...
  <div class="card"><div class="card-head"><h2>🔌 REST API</h2></div>
    <div style="padding:16px 20px"><div class="code-block" id="c4"><button class="copy-btn" onclick="cc('c4')">Copy</button>GET ${HOST}/api/check/lookup/{zipcode}

// Allowed: { "data": { "zip":"10001","result":"allow","allowed":true,"message":"..." } }
// Blocked: { "data": { "zip":"90210","result":"block","allowed":false,"message":"..." } }</div></div>
  </div>
</div>

<!-- ═══ APP BLOCK ═══ -->
<div class="page" id="page-appblock">
  <div class="page-header"><div class="page-title">App Block</div><div class="page-sub">Choose how and where the zip checker appears on your store.</div></div>
  <div class="card">
    <div class="card-head"><h2>🧩 Placement Mode</h2></div>
    <div class="appblock-grid">
      <div class="appblock-card selected" id="ab-auto" onclick="selectBlock('auto')">
        <div class="appblock-icon">⚡</div>
        <div class="appblock-name">Auto Placement</div>
        <div class="appblock-desc">Widget automatically appears above Add to Cart &amp; Buy Now on product pages. No theme edits needed.</div>
        <div style="margin-top:12px"><span style="background:var(--green-lt);color:var(--green-dk);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px">✅ Active</span></div>
      </div>
      <div class="appblock-card" id="ab-manual" onclick="selectBlock('manual')">
        <div class="appblock-icon">✏️</div>
        <div class="appblock-name">Manual Placement</div>
        <div class="appblock-desc">Use the embed code to place the widget exactly where you want it. Full control over position.</div>
        <div style="margin-top:12px"><span style="background:var(--g100);color:var(--g500);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px">Manual</span></div>
      </div>
      <div class="appblock-card" id="ab-cart" onclick="selectBlock('cart')">
        <div class="appblock-icon">🛒</div>
        <div class="appblock-name">Cart Page Block</div>
        <div class="appblock-desc">Show the zip checker on the cart page before checkout. Validates delivery before order placement.</div>
        <div style="margin-top:12px"><span style="background:var(--g100);color:var(--g500);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px">Cart</span></div>
      </div>
      <div class="appblock-card" id="ab-popup" onclick="selectBlock('popup')">
        <div class="appblock-icon">💬</div>
        <div class="appblock-name">Popup / Overlay</div>
        <div class="appblock-desc">Widget triggers as a popup when customers click Add to Cart. Requires Starter plan or above.</div>
        <div style="margin-top:12px"><span id="ab-popup-badge" style="background:var(--amber-lt);color:var(--amber);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px">🔒 Starter+</span></div>
      </div>
    </div>
    <div style="padding:0 20px 20px"><button class="btn btn-primary" onclick="saveBlock()">💾 Save Placement</button></div>
  </div>
  <div class="card">
    <div class="card-head"><h2>📋 Cart Button Behavior</h2></div>
    <div style="padding:20px;display:flex;flex-direction:column;gap:16px">
      <label style="display:flex;align-items:center;gap:12px;cursor:pointer;font-size:14px;font-weight:500;color:var(--g700)">
        <label class="toggle"><input type="checkbox" id="hide-cart-toggle" checked/><span class="slider"></span></label>
        Hide Add to Cart &amp; Buy Now until a valid zip is entered
      </label>
      <label style="display:flex;align-items:center;gap:12px;cursor:pointer;font-size:14px;font-weight:500;color:var(--g700)">
        <label class="toggle"><input type="checkbox" id="show-on-valid-toggle" checked/><span class="slider"></span></label>
        Show Add to Cart &amp; Buy Now when a valid zip is confirmed
      </label>
      <button class="btn btn-primary btn-sm" onclick="saveCartBehavior()" style="align-self:flex-start">Save Behavior</button>
    </div>
  </div>
  <div class="card">
    <div class="card-head"><h2>🛠 Theme Editor</h2></div>
    <div style="padding:20px">
      <p style="font-size:14px;color:var(--g700);margin-bottom:16px">For Online Store 2.0 themes, you can add the ZipCheck block directly from the Shopify Theme Editor.</p>
      <ol style="font-size:14px;color:var(--g600);line-height:2;padding-left:20px">
        ...
        <li>Search for <strong>"ZipCheck"</strong> and select it</li>
        <li>Position it above the Add to Cart button and save</li>
      </ol>
      <div class="info-box" style="margin-top:14px">💡 Auto Placement mode works even without theme editor access — it injects the widget via script tag. Compatible with all Shopify themes including dev stores.</div>
    </div>
  </div>
</div>
...
<div class="page" id="page-customcss">
  <div class="page-header"><div class="page-title">Custom CSS</div><div class="page-sub">Write CSS to customize the widget appearance beyond the built-in settings.</div></div>
  <div class="card">
    <div class="card-head"><h2>🎨 Custom CSS Editor</h2><button class="btn btn-primary btn-sm" onclick="saveCSS()">💾 Save CSS</button></div>
    <div style="padding:20px">
      <div style="font-size:12px;color:var(--g500);margin-bottom:10px">Target widget elements using <code style="background:var(--g100);padding:1px 6px;border-radius:4px;font-family:var(--mono)">[data-zipcheck]</code> as the parent selector.</div>
...
[data-zipcheck] .zc-inp {
  font-size: 16px;
}"></textarea>
      <div style="margin-top:12px;display:flex;gap:10px;align-items:center">
        <button class="btn btn-ghost btn-sm" onclick="document.getElementById('css-editor').value=''">🗑 Clear</button>
        <span style="font-size:12px;color:var(--g400)">CSS is injected into every page where the widget is active.</span>
      </div>
    </div>
  </div>
  <div class="card">
    ...
    <div style="padding:20px">
      <div class="code-block" style="margin:0"><span style="color:#f8d66d">[data-zipcheck] .zc-wrap</span>   <span style="color:#94a3b8">/* Widget container */</span>
<span style="color:#f8d66d">[data-zipcheck] .zc-lbl</span>    <span style="color:#94a3b8">/* Title label */</span>
<span style="color:#f8d66d">[data-zipcheck] .zc-inp</span>    <span style="color:#94a3b8">/* Zip code input field */</span>
<span style="color:#f8d66d">[data-zipcheck] .zc-btn</span>    <span style="color:#94a3b8">/* Check button */</span>
<span style="color:#f8d66d">[data-zipcheck] .zc-res</span>    <span style="color:#94a3b8">/* Result message area */</span>
<span style="color:#f8d66d">[data-zipcheck] .zc-row</span>    <span style="color:#94a3b8">/* Input + button row */</span></div>
    </div>
  </div>
</div>
...
<div class="page" id="page-pricing">
  <div class="page-header"><div class="page-title">Pricing Plans</div><div class="page-sub">Upgrade to unlock more zip codes, features, and support options.</div></div>
  <div class="pricing-toggle">
    <span class="ptl active" id="lbl-monthly">Monthly</span>
    <label class="billing-toggle"><input type="checkbox" id="billing-toggle" onchange="switchBilling(this.checked)"/><span class="billing-slider"></span></label>
  ...
  <div class="plans-grid">
    <div class="plan-card" id="pc-free">
      ...
      <button class="plan-btn plan-btn-free" id="pb-free" onclick="upgradePlan('free')">Get Started Free</button>
    ...
    <div class="plan-card" id="pc-basic">
      ...
      <button class="plan-btn plan-btn-basic" id="pb-basic" onclick="upgradePlan('basic')">Start 3-Day Free Trial</button>
      <hr class="plan-divider"/>
      <ul class="plan-features">
    ...
    <div class="plan-card popular" id="pc-starter">
      ...
      <button class="plan-btn plan-btn-starter" id="pb-starter" onclick="upgradePlan('starter')">Start 3-Day Free Trial</button>
      <hr class="plan-divider"/>
      <ul class="plan-features">
    ...
    <div class="plan-card" id="pc-pro">
      <div class="plan-ent-label">ENTERPRISE</div>
      ...
      <button class="plan-btn plan-btn-pro" id="pb-pro" onclick="upgradePlan('pro')">Start 3-Day Free Trial</button>
      <hr class="plan-divider"/>
      <ul class="plan-features">
...
<div class="page" id="page-faq">
  <div class="page-header"><div class="page-title">Frequently Asked Questions</div><div class="page-sub">Everything you need to know about ZipCheck.</div></div>
  <div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)"><span>How does the Zip Code Checker widget work?</span><span class="faq-arrow">▼</span></button><div class="faq-a">The widget is a lightweight snippet embedded on your Shopify store. When a customer enters their zip or postal code, it instantly checks against your allow/deny rules and shows a real-time delivery availability message — without any page reload.</div></div>
  <div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)"><span>How do I add or import zip codes?</span><span class="faq-arrow">▼</span></button><div class="faq-a">Add zip codes manually one at a time using the form in the Zip Rules tab. For bulk uploads (Starter plan+), use the Import feature which supports both CSV and Excel files. Your file needs at least a <code style="background:var(--g100);padding:1px 5px;border-radius:4px;font-family:var(--mono);font-size:12px">ZipCode</code> column.</div></div>
  <div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)"><span>Will the widget slow down my store?</span><span class="faq-arrow">▼</span></button><div class="faq-a">No. The widget script loads asynchronously and never blocks page rendering. The zip code check API call typically responds in under 100ms. Your store's Core Web Vitals are not affected.</div></div>
  <div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)"><span>Can I customize the widget colors and text?</span><span class="faq-arrow">▼</span></button><div class="faq-a">Yes, fully. Go to the Settings tab to customize button colors, success/error colors, widget title, placeholder text, button label, and the allow/deny messages. Use Custom CSS for advanced styling. All changes sync to your storefront when saved.</div></div>
  <div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)"><span>What happens if a zip code isn't in my list?</span><span class="faq-arrow">▼</span></button><div class="faq-a">If a customer enters a zip code that doesn't match any of your rules, the widget displays a neutral message: "No rule found. Please contact us." You can add more rules or use wildcard patterns (Starter plan+) to cover ranges like 1000* to match all 10000–10009 codes.</div></div>
  <div class="faq-item"><button class="faq-q" onclick="toggleFaq(this)"><span>How do I temporarily deactivate the app?</span><span class="faq-arrow">▼</span></button><div class="faq-a">Use the App Active toggle at the bottom of the left sidebar. When deactivated, the widget stops loading on your storefront immediately — no code removal needed. Simply toggle it back on to re-enable. All rules and settings are preserved.</div></div>
</div>
  ...
  <div class="modal-box">
    ...
    <div style="padding:18px 20px;overflow-y:auto;flex:1">
      <div id="imp-s1">
        ...
        <div id="drop-zone" onclick="document.getElementById('file-inp').click()" style="border:2px dashed var(--g300);border-radius:10px;padding:36px 20px;text-align:center;cursor:pointer;transition:all .2s;background:var(--g50)" ondragover="event.preventDefault();this.style.borderColor='var(--green)';this.style.background='var(--green-lt)'" ondragleave="this.style.borderColor='var(--g300)';this.style.background='var(--g50)'" ondrop="handleDrop(event)">
      ...
      <div id="imp-s2" style="display:none">
        ...
        <div style="margin-bottom:12px;display:flex;gap:10px;align-items:center;font-size:13px"><strong>Mode:</strong><label style="cursor:pointer;display:flex;align-items:center;gap:4px"><input type="radio" name="imp-mode" value="merge" checked/> Merge</label><label style="cursor:pointer;display:flex;align-items:center;gap:4px"><input type="radio" name="imp-mode" value="replace"/> Replace all</label></div>
        <div style="max-height:260px;overflow-y:auto;border:1px solid var(--g200);border-radius:8px"><table style="width:100%;border-collapse:collapse"><thead style="position:sticky;top:0;background:var(--g50)"><tr><th style="padding:7px 12px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--g500);text-align:left">Zip</th><th style="padding:7px 12px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--g500);text-align:left">Type</th><th style="padding:7px 12px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--g500);text-align:left">Message</th><th style="padding:7px 12px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--g500);text-align:left">Status</th></tr></thead><tbody id="imp-tbody"></tbody></table></div>
      </div>
    </div>
...
const PLAN_FEATURES = {
  free:    { bulk:false, popup:false, header:false, customCss:false, wildcard:false },
  basic:   { bulk:false, popup:true,  header:false, customCss:true,  wildcard:false },
  starter: { bulk:true,  popup:true,  header:true,  customCss:true,  wildcard:true },
  pro:     { bulk:true,  popup:true,  header:true,  customCss:true,  wildcard:true }
};
...
async function toggleApp(active) {
  try {
    ...
    document.getElementById('status-dot').className = 'status-dot' + (j.active ? '' : ' off');
    document.getElementById('status-text').textContent = j.active ? 'App Active' : 'App Inactive';
    toast(j.active ? '✅ App activated' : '⏸️ App deactivated', j.active ? 's' : '');
...
function renderRules(rules) {
  ...
  const msg    = document.getElementById('plan-limit-msg');
  if (rules.length >= limit) {
    banner.style.display = 'flex';
    msg.textContent = 'You\\'ve reached the ' + limit + ' zip code limit on the ' + currentPlan + ' plan. Upgrade to add more.';
...
async function addRule() {
  ...
  if (cnt >= limit) { toast('⚠️ Zip code limit reached. Upgrade your plan.', 'e'); return; }
...
function checkImportPlan() {
  if (!PLAN_FEATURES[currentPlan].bulk) {
    toast('🔒 Bulk import requires Starter plan or above. Upgrade to unlock.', 'e');
    return;
  }
  openImport();
}
...
function upv() {
  ...
  // Mobile preview
  document.getElementById('pvm-btn').style.background = btn;
  document.getElementById('pvm-btn').style.color = btxt;
...
async function saveSettings() {
  ...
  try { await fetch(API + '/api/settings', { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify(s) }); toast('💾 Settings saved!', 's'); } catch(e) { toast('Failed to save', 'e'); }
}
...
async function saveCSS() {
  ...
  try { await fetch(API + '/api/custom-css', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ css }) }); toast('💾 CSS saved!', 's'); } catch(e) { toast('Failed','e'); }
}

// ── APP BLOCK ─────────────────────────────────────────────────────────────────
let selectedBlock = 'auto';
function selectBlock(mode) {
  if (mode === 'popup' && !PLAN_FEATURES[currentPlan].popup) { toast('🔒 Popup mode requires Starter plan.', 'e'); return; }
  selectedBlock = mode;
  ['auto','manual','cart','popup'].forEach(m => { document.getElementById('ab-' + m).classList.toggle('selected', m === mode); });
}
function saveBlock() { toast('✅ Placement saved: ' + selectedBlock, 's'); }
function saveCartBehavior() { toast('✅ Cart behavior saved!', 's'); }
...
function switchBilling(isYearly) {
  ...
  ['basic','starter','pro'].forEach(plan => {
    document.getElementById('p-' + plan).textContent = PRICES[billingMode][plan].toFixed(2);
    document.getElementById('b-' + plan).innerHTML = isYearly ? 'Billed $' + YEARLY_BILLED[plan] + '/year' : '&nbsp;';
  });
}
function upgradePlan(plan) {
  ...
  toast('🚀 Switched to ' + plan.charAt(0).toUpperCase() + plan.slice(1) + ' plan!', 's');
  loadRules();
}
